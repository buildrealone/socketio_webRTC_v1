...
socket.on("accept_join", async (userObjArr) => {
  ...
  for (let i = 0; i < length - 1; ++i) {
    try {
      ...
      writeChat(`"${userObjArr[i].nickname}"` + "님이 방에 있습니다.", NOTICE_CN);
    } catch (err) {
      console.error(err);
    }
  }
  // writeChat("님이 방에 있습니다.", NOTICE_CN);
});
...
socket.on("offer", async (offer, remoteSocketId, remoteNickname) => {
  try {
    ...
    writeChat(`✅ "${remoteNickname}"님이 입장하셨습니다.`, NOTICE_CN);
  } catch (err) {
    console.error(err);
  }
});
...
socket.on("leave_room", (leavedSocketId, nickname) => {
  ...
  writeChat(`❌ "${nickname}"님이 퇴장하셨습니다.`, NOTICE_CN);
  --peopleInRoom;
  sortStreams();
});
